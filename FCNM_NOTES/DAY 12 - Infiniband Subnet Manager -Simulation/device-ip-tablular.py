...
def get_network_devices(base_url, service_ticket):
    """Retrieve the list of network devices and print their IP addresses in a table."""
    ...
    if response.status_code == 200:
        ...
        table = PrettyTable()
        table.field_names = ["Device Name", "IP Address"]
        
        # Add devices to the table
        for device in devices.get('response', []):
            table.add_row([device.get("hostname", "N/A"), device.get("managementIpAddress", "N/A")])
